...
struct WifiApConfig
{
  ...
  WifiApConfig(const char *ssid_, const char *password_ = "",
               uint8_t ch = 1, bool hide = false, uint8_t maxConn = 4)
      : channel(ch), hidden(hide), maxConnections(maxConn)
  {
    strncpy(ssid, ssid_, sizeof(ssid) - 1);
    strncpy(password, password_, sizeof(password) - 1);
  }
};
